  ...
  public postCitySubmitted() {
    ...
    this.citiesService.postCity(this.postCityForm.value).subscribe({
      next: (response: City) => {
        ...
        // this.loadCities();
        this.cities.push(new City(response.cityID, response.cityName)); //better not to reload cities from server
        this.putCityFormArray.push(
          new FormGroup({
    ...
    this.citiesService
      ...
      .subscribe({
        next: (response: string) => {
          ...
          this.putCityFormArray.controls[i].reset(
            this.putCityFormArray.controls[i].value
          );
        },
        error: (error: any) => {
          console.log(error);
